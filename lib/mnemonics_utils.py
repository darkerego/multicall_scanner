...
# LEDGER_ETH_DERIVATION_PATH = "M/44H/60H/0H/0"
alt_paths = ["m/0'/0'", "m/44'/60'/0'/0", "m/44'/60'/0'/0", "m/0'/0", "m/44'/0'/0'", "m/49'/0'/0'/0", "m/84'/0'/0'/0",
             "m/44'/60'/0'", "m/44'/60'/0/0", "m/0'/60'/0'/0", "m/44'/60'/0'/0"]
...
def b58xprv(parent_fingerprint, private_key, chain, depth, childnr):
    ...
    raw = (b'\x04\x88\xad\xe4' + bytes(chr(depth), 'utf-8') + parent_fingerprint + childnr.to_bytes(4,
                                                                                                    byteorder='big') + chain + b'\x00' + private_key)
...
def b58xpub(parent_fingerprint, public_key, chain, depth, childnr):
    ...
    raw = (b'\x04\x88\xb2\x1e' + bytes(chr(depth), 'utf-8') + parent_fingerprint + childnr.to_bytes(4,
                                                                                                    byteorder='big') + chain + public_key)
...
def generate(mnemonic, path, passphrase="", children=3):
    print('PATH: ', path)
...
if __name__ == '__main__':
    ...
    args.add_argument('-f', '--file', type=str, required=True, help='The file with memnonic')
    args.add_argument('-c', '--children', type=int, default=3, help='Show the next n child keys.')
    args.add_argument('-ep', '--extended-paths', dest='extended_paths', help='Try extensive derivation path list.')
